...
for i in $(seq $N); do
  ...
  sed -i 's/.*IMSI number/    supi: "'$IMSI'"  # IMSI number/g' ../charts/new-ue/values.yaml
  
  # install UEi
  # NOTE: for every new ue we need a related configmap
  echo "Installing UE$i..."
  helm -n 5g install ue$i ../charts/new-ue --set ue.configmap.name=ue-configmap$i
  
  # NOTE: if you need to run the UE on a particular node of your cluster you can install it like this
  #helm -n 5g install ue$i charts/new-ue --set ue.configmap.name=ue-configmap$i ue.nodeSelector."kubernetes\.io/hostname"<NAME>"
...
sed -i 's/supi: "'$IMSI'"  # IMSI number/    supi: "imsi-2089300000001"  # IMSI number/g' ../charts/new-ue/values.yaml 
