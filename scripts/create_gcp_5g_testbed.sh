...
for node in "gcp-master" "gcp-1" "gcp-2"; do
	...
	ssh $node /bin/bash << EOF 
        ...
        sudo sed -i 's/GRUB_DEFAULT=0/GRUB_DEFAULT="Advanced options for Ubuntu 20.04.5 LTS (20.04) (on /dev/sda1)>Ubuntu, with Linux 5.8.0-1039-gcp (on /dev/sda1)"/g' /etc/default/grub.d/50-cloudimg-settings.cfg
        sudo update-grub2
        sudo reboot
EOF
done
